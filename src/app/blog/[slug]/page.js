    ...
    <main className={styles.articlePage}>
      ...
      <div className={styles.articleLayout}>
        <article className={styles.articleContent}>
          ...
          <section className={styles.tutorial}><p className={styles.label}>TUTORIAL</p><h2>How to get started in three considered steps</h2><ol>{post.tutorial.map((step) => <li key={step}>{step}</li>)}</ol><div className={styles.tutorialImage}><Image src="/images/blog/digital-invitation-tutorial.webp" alt="Laptop and phone used to plan a digital wedding invitation" fill sizes="(max-width: 820px) 100vw, 720px" /></div><p>Use the <Link href="/collections">FOLDÈ collection preview</Link> to choose a direction, then open your live invitation preview to check the experience from a guest&apos;s perspective.</p></section>
          {post.sections.map((section) => <section key={section.heading}><h2>{section.heading}</h2><p dangerouslySetInnerHTML={{ __html: section.body }} /></section>)}
          <figure className={styles.inlineImage}><Image src={post.secondaryImage} alt={`${post.keyword} inspiration`} fill sizes="(max-width: 820px) 100vw, 720px" /><figcaption>Invitation inspiration by FOLDÈ Wedding.</figcaption></figure>
        ...
        <aside className={styles.articleAside}><p>PLAN YOUR INVITATION</p><h2>One elegant link for every guest detail.</h2><Link className={styles.primaryButton} href="/">Explore FOLDÈ</Link><Link className={styles.asideLink} href="/packages">Compare packages →</Link></aside>
      </div>
