    ...
    return (
      <div style={{ backgroundColor: '#F5F0E8', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-body)', color: '#3D2B1F', padding: '2rem' }}>
        <div style={{ backgroundColor: '#fff', borderRadius: '24px', padding: '4rem 3rem', maxWidth: '500px', width: '100%', textAlign: 'center', boxShadow: '0 8px 32px rgba(0,0,0,0.04)', border: '1px solid rgba(0,0,0,0.06)' }}>
          ...
          <h1 style={{ fontSize: '1.6rem', fontFamily: 'var(--font-heading)', color: '#3D2B1F', marginBottom: '0.5rem' }}>Confirmando el pago</h1>
          <p style={{ color: '#888', fontSize: '0.95rem' }}>Espera mientras verificamos tu pago...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div style={{ backgroundColor: '#F5F0E8', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-body)', color: '#3D2B1F', padding: '2rem' }}>
        <div style={{ backgroundColor: '#fff', borderRadius: '24px', padding: '4rem 3rem', maxWidth: '500px', width: '100%', textAlign: 'center', boxShadow: '0 8px 32px rgba(0,0,0,0.04)', border: '1px solid rgba(0,0,0,0.06)' }}>
          ...
          <h1 style={{ fontSize: '2rem', fontFamily: 'var(--font-heading)', color: '#3D2B1F', marginBottom: '1rem' }}>Problema con el pago</h1>
          <p style={{ color: '#666', fontSize: '1rem', lineHeight: 1.6, marginBottom: '2rem' }}>
            No hemos podido verificar tu pago. Si crees que se trata de un error, escríbenos a folde.wedding@gmail.com.
          </p>
  ...
  return (
    <div style={{ backgroundColor: '#F5F0E8', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-body)', color: '#3D2B1F', padding: '2rem' }}>
      <div style={{ backgroundColor: '#fff', borderRadius: '24px', padding: '4rem 3rem', maxWidth: '500px', width: '100%', textAlign: 'center', boxShadow: '0 8px 32px rgba(0,0,0,0.04)', border: '1px solid rgba(0,0,0,0.06)' }}>
        ...
        <h1 style={{ fontSize: '2.2rem', fontFamily: 'var(--font-heading)', color: '#3D2B1F', marginBottom: '1rem' }}>¡Pago validado!</h1>
        <p style={{ color: '#666', fontSize: '1.05rem', lineHeight: 1.6, marginBottom: '2.5rem' }}>
          {isCustom
            ? `¡Bienvenido/a ${orderData?.name || ''}! Tu pedido ${orderData?.plan} está confirmado. Completa el cuestionario de personalización para que podamos empezar a crear tu diseño.`
            : `Tu pedido se ha confirmado correctamente. ¡Bienvenido/a a FOLDÈ Design! Ya puedes acceder a tu panel privado y empezar a personalizar tu invitación.`
          }
        </p>
        <Link href={isCustom ? `/checkout?step=4&plan=${orderData?.planId}&slug=${orderData?.slug || ''}` : "/dashboard"} style={{ display: 'inline-block', width: '100%', backgroundColor: '#5C3A1E', color: '#fff', padding: '1rem', borderRadius: '12px', fontWeight: 600, textDecoration: 'none', transition: 'background 0.2s', letterSpacing: '1px' }}>
          {isCustom ? "COMPLETAR EL CUESTIONARIO →" : "ACCEDER A MI PANEL →"}
        </Link>
      </div>
    </div>
  );
